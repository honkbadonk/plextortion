...
def create_ransom():
    ...
    # Find which library the prerequisite movie is in
    server, account, sections = get_plex_connection()
    prereq_library = None
    if sections:
        for section in sections:
            if section.type == 'movie':
                for movie in section.all():
                    if movie.title == prerequisite:
                        prereq_library = section.title
                        break
    
    # Don't allow locking the same library
    if prereq_library == locked_library:
        print(f"⚠️ Cannot lock {locked_library} - that's where {prerequisite} lives!")
        return redirect(url_for('index'))
